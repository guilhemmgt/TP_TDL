...
let rec value_of_expr expr env =
  ...
  and
    ...
    (* A compléter *)
    let fexpr = (value_of_expr _fexpr _env) in
    match fexpr with
    | FrozenValue (frozen_expr, frozen_env) ->
      (
      match frozen_expr with
      | FunctionNode (ident, e) -> 
        (
        let pexpr = (value_of_expr _pexpr _env) in
          match pexpr with
          | (ErrorValue _) as result -> result
          | _ ->
            (
            let _newenv = (ident, pexpr)::frozen_env in
              let res = (value_of_expr e _newenv) in
                (match res with
                | (ErrorValue _) as result -> result
                | _ -> res)
            )
  ...
  and
    ...
    (* A compléter *)
    let node = LetrecNode (_ident, _bvalue, _bvalue) in
      let frozen = FrozenValue (node, _env) in
        let newenv = (_ident, frozen)::_env in
          value_of_expr _bin newenv
